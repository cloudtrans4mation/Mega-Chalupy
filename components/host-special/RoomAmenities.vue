            ...
            <div class="flex flex-col w-full">
              ...
              <section v-for="(section, sectionIndex) in amenities" :key="sectionIndex" class="mt-4">
                <h2 class="flex gap-4 items-center py-2.5 w-full text-base">
                  <img loading="lazy" :src="section.icon" alt="" class="object-contain shrink-0 self-stretch my-auto aspect-square w-[30px]" />
                  <span>{{ section.title }}</span>
                </h2>
                <ul class="list-none p-0">
                  <li v-for="(amenity, amenityIndex) in section.amenities" :key="amenityIndex" class="flex justify-between items-center mt-4 w-full max-md:flex-col">
                    <span class="flex-1">{{ amenity.name }}</span>
                    <label class="checkbox-label">
                      <input type="checkbox" class="checkbox-input" :value="amenity.name" @change="toggleAmenity(amenity.name, $event.target.checked)">
                      <span class="checkbox-custom">
                        <img loading="lazy" :src="amenity.image" alt="" class="object-contain shrink-0 self-stretch my-auto aspect-square w-[30px] mt-2 md:mt-0">
