...
const { type = 'text', currency = 'CZK' } = defineProps<InputProps>();
...
<template>
  <div class="relative w-full flex items-center space-x-2">
    ...
    <div class="relative flex-1">
      <input :class="`peer w-full p-4 pt-6 font-light bg-white border-2 rounded-md outline-none transition disabled:opacity-70 disabled:bg-gray-300 disabled:border-0 disabled:cursor-not-allowed disabled:text-gray-500 ${!!error ? 'border-rose-500' : 'focus:border-black'
        }`" :id="id" :type="type" :disabled="disabled" placeholder=" " v-model="model" />
      <!-- Label -->
      <label :class="`absolute text-md duration-150 transform -translate-y-3 top-5 z-10 origin-[0] left-4 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4 ${!!error ? 'text-rose-500' : 'text-zinc-400'
        }`">
        {{ label }}
      </label>
    </div>
