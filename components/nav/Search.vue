...
<template>
    ...
    <button @click="openModal" class="show-map-button">
      Show the cart
          ...
          <div class="flex items-center justify-center min-h-full p-4 text-center">
            <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
              ...
              <DialogPanel
                ...
                <div class="flex items-center justify-center py-4 rounded-t relative border-b-[1px]">
                  <h3 class="text-3xl font-bold">Filters</h3>
                  <button @click="closeModal" class="absolute p-1 transition border-0 hover:opacity-70 right-2">
                    <Icon name="heroicons-solid:x" class="size-4" />
                  </button>
                </div>

                <div v-if="step === STEPS.LOCATION">
                  <div class="flex flex-col gap-8 mt-2 z-[10000]">
                    <Heading title="Where do you wanna go?" subTitle="Pick a country" />
                    <div class="py-2">
                      <CountrySelect :selectedCountry="location" @countrySelect="countrySelect" />
                    </div>
                    <Map :center="location?.latlng" />

                    <div class="flex flex-col gap-4 md:flex-row">
                      <Button label="Close" outline @click="isOpen = false" />
                      <Button style="background-color: blue;" @click="onNext" label="Next" />
                    </div>
                  </div>
                </div>
                <div v-if="step === STEPS.DATE">
                  <div class="flex flex-col gap-8 mt-2">
                    ...
                    <Calendar v-model="dateRange" expanded />
                    <div class="flex flex-col gap-4 md:flex-row">
                ...
                <div v-if="step === STEPS.INFO">
                  <div class="flex flex-col gap-8 mt-2">
                    ...
                    <Counter title="Guests" subtitle="How many guests are coming ?" :value="guestCount" @add="add"
                      @reduce="reduce" />
                    <hr />
                    <Counter title="Rooms" subtitle="How many room are coming ?" :value="roomCount" @add="add"
                      @reduce="reduce" />
                    <hr />

                    <Counter title="Bathrooms" subtitle="How many bathrooms are coming ?" :value="bathroomCount"
                      @add="add" @reduce="reduce" />
                    <hr />

                    <div class="flex flex-col gap-4 md:flex-row">
                      <Button label="Back" outline @click="onBack" />
                      <Button style="background-color: blue;" @click="onSubmit" label="Submit" />
                    </div>
                  </div>
                </div>
              </DialogPanel>
